...
final class CaptureCoordinator: NSObject, ObservableObject, SCStreamOutput {
  ...
  func start() async throws {
    ...
    guard let mainDisplay = displays.first else {
      throw NSError(
        domain: "CaptureError", code: 1, userInfo: [NSLocalizedDescriptionKey: "No displays found"])
    }
    ...
    stream = SCStream(
      filter: SCContentFilter(display: mainDisplay, excludingWindows: excludedWindows),
      configuration: config,
      delegate: self)

    guard let stream = stream else {
      throw NSError(
        domain: "CaptureError", code: 2,
...
final class OptimizedMetalRenderer: NSObject, MTKViewDelegate {
  ...
  func draw(in view: MTKView) {
    ...
    var shaderParams = ShaderParams(
      ...
      distortion_fac: simd_float2(1.0, 1.0), scale_fac: simd_float2(1.0, 1.0), feather_fac: 0.0,
      noise_fac: 0.0, bloom_fac: 0.0, crt_intensity: 0.0, glitch_intensity: 0.0, scanlines: 0.0)
